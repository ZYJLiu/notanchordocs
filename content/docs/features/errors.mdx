...
type used in Anchor programs is a type alias that wraps the standard Rust
...
An `AnchorError` can be thought of as two categories:

1. Internal Anchor Errors - These are built-in errors defined by the Anchor
   framework itself, used for common validation and runtime checks. They are
   defined in the
   [`ErrorCode`](https://github.com/coral-xyz/anchor/blob/v0.30.1/lang/src/error.rs#L10-L275)
   enum.

2. Custom Program Errors - These are program-specific errors that developers can
   define to handle their own error cases. They allow programs to communicate
   specific error conditions back to clients.
...
`error_code` attribute macro. The implementation details can be found
