...
Zero copy is a feature that is useful for working with large account data.

To use zero-copy add the `bytemuck` crate to your dependencies. Enable the
`min_const_generics` feature to work with arrays of arbitrary sizes.
...
<Callout type="info">
  ...
  CPI limitations. The `init` constraint makes a CPI call to the SystemProgram
...
to get a mutable reference to the account data. Under the hood, `load_init` also
...
For accounts larger than 10240 bytes, use the
...
been initialized by checking that its discriminator is unset.
...
With the `zero` constraint, you'll need to create the account in a separate
instruction by directly calling the System Program. This allows you to create
accounts up to Solana's maximum account size of 10MB (10_485_760 bytes),
bypassing the CPI size limitation.
