...
title: Emit Events
description:
  Learn how to emit events in Anchor programs using emit!() and emit_cpi!()
  macros.
---
...
1. `emit!()` - Emits events directly to program logs, which is simpler but can
   potentially be truncated
2. `emit_cpi!()` - Emits events through Cross Program Invocation (CPI)
   instruction data, which is more reliable but has higher compute costs
...
attribute must be added to the `#[derive(Accounts)]` struct used in an
...
included in the `data` field of an inner instruction. The inner instruction
appears under `meta.innerInstructions[0].instructions[0]` in the transaction
data.
...
<Callout type="info">
  There is currenly not a way to subscribe to event data emitted through a CPI.
  It requires fetching the transaction data and decoding the event data from the
  instruction data.
</Callout>

https://github.com/coral-xyz/anchor/tree/master/tests/events

https://github.com/anza-xyz/agave/blob/v1.18.26/sdk/program/src/log.rs#L115-L124

https://github.com/anza-xyz/agave/blob/v1.18.26/sdk/program/src/syscalls/definitions.rs#L62

https://github.com/anza-xyz/agave/blob/v1.18.26/programs/bpf_loader/src/syscalls/mod.rs#L468

https://github.com/anza-xyz/agave/blob/v1.18.26/programs/bpf_loader/src/syscalls/logging.rs#L107-L160

https://github.com/anza-xyz/agave/blob/v1.18.26/program-runtime/src/stable_log.rs#L46-L61
