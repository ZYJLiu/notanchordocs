...
This example demonstrates how to use `anchor-client` to interact with an Anchor
program. The program client is automatically generated from the program's IDL
using the `declare_program!` macro. This macro generates a depedency free client
interface that allows you to interact with the program's instructions and
accounts.

This example uses the IDL from the following example counter program:
...
Example folder structure for client rust script:
...
The program IDL must be in an `/idls` folder. The `declare_program!` macro
searches for the IDL in the `/idls` folder to generate the client interface.
...
1. The `declare_program!` macro - Generates a client interface for the program
   based on the IDL
...
Dependencies for the example client script:
