...
Program
[source code](https://github.com/solana-labs/solana-program-library/tree/master/token/program-2022/src/extension).

Each extension adds specific state data that must be initialized during mint or
token account creation. When initializing either type of account, you can enable
...
[`ExtensionType`](https://github.com/solana-labs/solana-program-library/blob/master/token/program-2022/src/extension/mod.rs#L1054-L1139)
...
Let's examine the `ExtensionType` enum below to understand the available
extensions:

```rs title="Token Extensions"
...
Each extension adds specialized functionality by including additional state data
that must be initialized when creating a mint or token account. All extension
specific state is stored in the in the
[`tlv_data`](https://github.com/solana-labs/solana-program-library/blob/master/token/program-2022/src/extension/mod.rs#L541-L549)
