...
<Callout type="info">
  ...
  type. Both the [Token
  Program](https://github.com/solana-labs/solana-program-library/blob/master/token/program/src/state.rs#L89-L110)
  and [Token Extension
  Program](https://github.com/solana-labs/solana-program-library/blob/master/token/program-2022/src/state.rs#L104-L124)
...
1. A specific mint (the token type it can hold)
...
You can view details about this token account on
...
is a PDA derived and created by the
[Associated Token Program](https://github.com/solana-labs/solana-program-library/tree/master/associated-token-account/program).
...
[here](https://github.com/solana-labs/solana-program-library/blob/master/associated-token-account/client/src/address.rs#L62-L69).

```rs title="Associated Token Account Address Derivation"
...
<Callout type="info">
  ...
  created account is owned by and has the same `Account` type structure as
  defined in the token program. The Associated Token Program itself maintains no
  state - it simply provides a standardized way to create token accounts at a
...
The `InterfaceAccount` type is a wrapper that allows the account to work with
both the Token Program and Token Extension Program. When used with the
`TokenAccount` type, it enables the field to accept a token account from either
program.
...
- Use `token` constraints when working with token accounts that are not
  ...
  are public keys from a standard keypair

The appropriate constraint set depends on your specific use case - ATAs are
recommended for user wallets, while custom token accounts are useful for
...
```rs title="Create Associated Token Account"
#[account(
    init,
    payer = <payer>,
    associated_token::mint = <mint>,
...
```rs title="Create Token Account with Keypair Public Key as Address"
#[account(
    init,
    payer = <payer>,
    token::mint = <mint>,
...
```rs title="Create Token Account with PDA as Address"
#[account(
    init,
    payer = <payer>,
    token::mint = <mint>,
